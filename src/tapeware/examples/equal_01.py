...
"""
Simple example: Create a Turing machine that accepts strings with equal number of 0s and 1s.
"""


def delta(state: str, symbol: str) -> tuple[str, str, str] | None:
    """
    Turing machine that recognises context-free language of strings with equal number of 0s and 1s.

    - It is not regular because it does not satisfy the pumping lemma for regular languages.
    - It is context-free because it can be generated by a context-free grammar (e.g. S → 0S1 | 1S0 | SS | ε).
